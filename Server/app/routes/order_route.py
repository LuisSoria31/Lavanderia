...
def create_order():
    data = request.json
    splited_date = data["estimated_delivety_date"].split("-")
    date = datetime(int(splited_date[0]), int(splited_date[1]), int(splited_date[2]))
    order = create_order(
...
def get_order_detail_endpoint(order_id):
    try:
        ...
        return jsonify({"msg": "Detalle de orden obtenido", "order":order})
    except Exception as e:
        return jsonify({"msg": "Ocurrio un error", "error":e}), 404